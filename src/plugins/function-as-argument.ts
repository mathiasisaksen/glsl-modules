...
function findHigherOrderFunctions(candidates: Array<ModuleEntity>) {
  return candidates.filter((entity) => isHigherOrderFunction(entity)) as Array<Func>;
}

function transformFunctionalArguments(argumentStrings: Array<string>) {
...
function determineNewFunctions(moduleEntities: Array<ModuleEntity>, context: PluginContext) {
  const newFunctions: Array<NewFunction> = [];

  const functionUsageCount: Record<string, number> = {};
  const memoizedHOFs: Record<string, Array<Func>> = {};
...
function createFunctionalArgumentValueMap(functionalArguments: Array<Argument>, callValues: Array<string>) {
...
function createHOFInstance(func: Func, newName: string, index: number, functionalArgNameValueMap: Record<string, string>, context: PluginContext) {
  ...
  let functionArgumentDependencies: Array<EntityDependency> = [];
